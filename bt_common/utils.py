...
def parse_system_monitor_data(data: dict, server_name: str) -> dict:
    ...
    # 解析CPU数据
    # cpu格式: [usage率%, 核心数, [用户态%, 系统态%], CPU型号, ?, ?]
...
def parse_proxy_site(proxy: dict, server_name: str) -> dict:
    ...
    Returns:
        格式化后的反代项目信息
    """
    # 反代项目通过status字段和healthy字段判断status
    # status: "1" = 运行, "0" = 停止
    # healthy: 1 = 健康, 0 = 不健康
